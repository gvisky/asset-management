...
function mapColumns(headerRow) {
  ...
  let y2025col = -1;   // total-only fallback (round-tripped export has no monthly 2025)
...
function rowsToLines(rows) {
  ...
  if (!map) return null;
  const { dim, m2025, m2026b, m2026af, y2025col } = map;
  const at = (r, i) => (i >= 0 ? r[i] : '');
  const series = (r, idxs) => idxs.map(i => num(at(r, i)));
  const has2025Monthly = m2025.some(i => i >= 0);
  const out = [];
  for (let i = headerRow + 1; i < rows.length; i++) {
    ...
    const y2025_actual = has2025Monthly ? sum(a2025) : num(at(r, y2025col));
