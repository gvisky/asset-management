...
function render() {
  ...
  // GL tables
  const byGL = {};
  items.forEach(it => { const k = it.gl_tr_no || '(none)'; (byGL[k] = byGL[k] || { alts: new Set(), jb: 0, ja: 0 }); if (it.sub_project) byGL[k].alts.add(it.sub_project); byGL[k].jb += it.ja_budget; byGL[k].ja += it.ja_actual; });
